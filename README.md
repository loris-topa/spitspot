...
4) When the container is up and running, execute `http -v POST  localhost:8000/init filename={name-of-your-choice} lat1={x1}  lon1={y1} lat2={x2} lon2={y2}` on your host machine.
    ...
    -d "lon2={y2}"
    ```
   Supposing you downloaded w49060_s10.zip and placed the geotiff in data-sources, with the name `source-bismantova.tif`, you can run this init command:
   ```http -v POST  127.0.0.1:8000/init filename=bismantova lat1=44.428633495773006 lon1=10.404637169518928 lat2=44.41019223908658 lon2=10.425157949161745```
    The initialization might take a long time, depending on the size of the area. The example provided above shouldn't take more than a few minutes.
...
6) Now you can start searching for spots in the loaded area. Well known example:
    A 2.5 slope scan will find most of the iconic lines in the Bismantova area. To start the scan, click the button on the top right of the map with the radar icon:
